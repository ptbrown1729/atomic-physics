"""
Calculate light-shifts of lithium-6 in far-off resonant dipole trap

This script is a work in progress, and is not fully tested
"""
import numpy as np
...
d2_redj, d_redl = mel.gamma2matrixel_Dline(lambda_d2, 1.5, gamma_d2, convention="wigner")
d1_redj, d_redl_check = mel.gamma2matrixel_Dline(lambda_d1, 0.5, gamma_d1, convention="wigner")
...
I = 1
lshifts, pols, gammas = mel.get_lightshift_Dline(b_fields, lambda_d1, lambda_d2, lambda_laser, intensity,
...
energies_g, eigvects_uncoupled_g, eigvects_coupled_g = mel.breit_rabi(b_fields, params_g, qnumbers_g)
coupled_states_g = mel.get_coupled_basis(qnumbers_g)
ng = eigvects_coupled_g.shape[0]

# J = 0.5 excited state
energies_d1, eigvects_uncoupled_d1, eigvects_coupled_d1 = mel.breit_rabi(b_fields, params_d1, qnumbers_d1)
coupled_states_d1 = mel.get_coupled_basis(qnumbers_d1)
nd1 = eigvects_coupled_d1.shape[0]

# J = 1.5 excited state
energies_d2, eigvects_uncoupled_d2, eigvects_coupled_d2 = mel.breit_rabi(b_fields, params_d2, qnumbers_d2)
coupled_states_d2 = mel.get_coupled_basis(qnumbers_d2)
nd2 = eigvects_coupled_d2.shape[0]

# semiclassical approximation neglecting spin dependence
ls_semi, gamma_semi = mel.get_lightshift_semiclass(lambda_d, 0.5 * (gamma_d1 + gamma_d2), lambda_laser, intensity)
ls_semi = ls_semi / 6.626e-34 / 1e6

# [gs, gl], [ss, ll, jj]
gj = mel.get_gj([gs, gl], [0.5, 0, 0.5])
# [gj, gi], [jj, ii, ff]
gf_half = mel.get_gf([gj, gi], [0.5, 1, 0.5])
ls_semi_fhalf, _, basis_half = mel.get_all_lightshift_semiclass(lambda_d1, lambda_d2, gamma_d1,
                                                                lambda_laser, intensity, gf_half, 0.5)

gf_thalf = mel.get_gf([gj, gi], [0.5, 1, 1.5])
ls_semi_fthreehalf, _, basis_threehalf = mel.get_all_lightshift_semiclass(lambda_d1, lambda_d2, gamma_d1,
                                                                          lambda_laser, intensity,
                                                                          gf_thalf, 1.5)
...
plt.title('Differential lightshift magnitude between |1>, |2>, and |3> Vs. Bfield\n'
          'Detuning=%0.2f GHz from D1 line, intensity = %0.2f W/m**2' %
          (laser_detune/1e9, intensity))
plt.legend(['13', '12', '23'])